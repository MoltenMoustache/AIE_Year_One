...
Button::Button(float x, float y, float width, float height)
{
	...
	m_buttonText[0] = '\0';
	m_font = nullptr;
	m_buttonTextureON = nullptr;
	m_buttonTextureOFF = nullptr;
}
...
Button::Button(aie::Texture* a_buttonTextureON, aie::Texture* a_buttonTextureOFF, float x, float y, float width, float height)
{
	//Store the button's position, width, and height.
	m_posX = x;
	m_posY = y;
	...
	m_buttonTextureON = a_buttonTextureON;
	m_buttonTextureOFF = a_buttonTextureOFF;
...
void Button::draw(aie::Renderer2D* renderer, float r, float g, float b, int a_stringLength)
{
	...
	if (m_buttonText[0] != '\0')
	{
		...
		// plus 0.5 and then finally multiplied by the font size
...
void Button::draw(aie::Renderer2D* renderer, bool a_isOnTexture)
{
	// Set the render colour to White
	renderer->setRenderColour(1.0f, 1.0f, 1.0f, 1.0f);
	if (a_isOnTexture) {
		renderer->drawSprite(m_buttonTextureON, m_posX, m_posY, m_width, m_height, 0, 1);
	}
	else {
		renderer->drawSprite(m_buttonTextureOFF, m_posX, m_posY, m_width, m_height, 0, 1);
	}
}
